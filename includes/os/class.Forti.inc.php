...
class Forti extends Linux
{
    ...
    private function getSystemStatus()
    {
        if ($this->_sysstatus === null) {
            if (CommonFunctions::executeProgram('echo', 'get system status | sshpass -p \''.PSI_FGT_PASSWORD.'\' ssh -T -o \'StrictHostKeyChecking=no\' '.PSI_FGT_USER.'@'.PSI_FGT_HOSTNAME.' -p '.PSI_FGT_PORT, $resulte, false) && ($resulte !== "")) {
    ...
    private function getSystemPerformance()
    {
        if ($this->_sysperformance === null) {
            if (CommonFunctions::executeProgram('echo', 'get system performance status | sshpass -p \''.PSI_FGT_PASSWORD.'\' ssh -T -o \'StrictHostKeyChecking=no\' '.PSI_FGT_USER.'@'.PSI_FGT_HOSTNAME.' -p '.PSI_FGT_PORT, $resulte, false) && ($resulte !== "")) {
                if (preg_match('/\$ (.+)/', $resulte, $resulto, PREG_OFFSET_CAPTURE)) {
    ...
    {
        if (CommonFunctions::executeProgram('echo', 'get hardware memory | sshpass -p \''.PSI_FGT_PASSWORD.'\' ssh -T -o \'StrictHostKeyChecking=no\' '.PSI_FGT_USER.'@'.PSI_FGT_HOSTNAME.' -p '.PSI_FGT_PORT, $resulte, false) && ($resulte !== "")) {
            if (preg_match('/\$ (.+)/', $resulte, $resulto, PREG_OFFSET_CAPTURE)) {
    ...
    {
        $netdevs = array();
        if (CommonFunctions::executeProgram('echo', 'diagnose ip address list | sshpass -p \''.PSI_FGT_PASSWORD.'\' ssh -T -o \'StrictHostKeyChecking=no\' '.PSI_FGT_USER.'@'.PSI_FGT_HOSTNAME.' -p '.PSI_FGT_PORT, $resulte, false) && ($resulte !== "")) {
            if (preg_match('/\$ (.+)/', $resulte, $resulto, PREG_OFFSET_CAPTURE)) {
        ...
        if (CommonFunctions::executeProgram('echo', 'diagnose ipv6 address list | sshpass -p \''.PSI_FGT_PASSWORD.'\' ssh -T -o \'StrictHostKeyChecking=no\' '.PSI_FGT_USER.'@'.PSI_FGT_HOSTNAME.' -p '.PSI_FGT_PORT, $resulte, false) && ($resulte !== "")) {
            if (preg_match('/\$ (.+)/', $resulte, $resulto, PREG_OFFSET_CAPTURE)) {
                $strBuf = substr($resulte, $resulto[1][1]);
            } else {
                $strBuf = $resulte;
    ...
    {
        if (CommonFunctions::executeProgram('echo', 'get hardware cpu | sshpass -p \''.PSI_FGT_PASSWORD.'\' ssh -T -o \'StrictHostKeyChecking=no\' '.PSI_FGT_USER.'@'.PSI_FGT_HOSTNAME.' -p '.PSI_FGT_PORT, $resulte, false) && ($resulte !== "")) {
            if (preg_match('/\$ (.+)/', $resulte, $resulto, PREG_OFFSET_CAPTURE)) {
