...
class SMART extends PSI_Plugin
{
    ...
    public function __construct($enc)
    {
        ...
        switch (strtolower(PSI_PLUGIN_SMART_ACCESS)) {
            case 'wmi':
                if ((PSI_OS == 'WINNT') || ((PSI_OS == 'Linux') && (defined('PSI_PLUGIN_SMART_WMI_HOSTNAME') || defined('PSI_WMI_HOSTNAME')))) {
                ...
                break;
            case 'command':
                if (((PSI_OS != 'WINNT') && (PSI_OS != 'Linux')) || (!defined('PSI_PLUGIN_SMART_WMI_HOSTNAME') && !defined('PSI_WMI_HOSTNAME'))) foreach ($disks as $disk) {
                    if (trim($disk) != "") {
                        $diskdev = "";
