...
//                alert("--" + data.Status + "--");
    ...
    if (data.Plugins.Plugin_MDStatus !== undefined) {
        ...
        if (data.Plugins.Plugin_MDStatus.Unused_Devices !== undefined) {
            var uitems = items(data.Plugins.Plugin_MDStatus.Unused_Devices);
            if ((uitems.length > 0) && (uitems[0]["@attributes"].Devices !== undefined)) {
                html = "<tr><th>" + genlang(15, true, "mdstatus") + "</th><th>" + uitems[0]["@attributes"].Devices + "</th></tr>";
                $('#mdstatus-data').append(html);
            }
        }
    } else {
